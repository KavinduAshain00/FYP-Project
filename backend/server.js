...
mongoose
  ...
  .then(async () => {
    ...
    try {
      const legacyCount = await mongoose.connection.db
        .collection("admins")
        .countDocuments();
      if (legacyCount > 0) {
        const legacyAdmins = await mongoose.connection.db
          .collection("admins")
          .find({})
          .toArray();
    ...
    if (adminCount === 0 && process.env.ADMIN_BOOTSTRAP_EMAIL) {
      ...
      if (email && email.includes("@")) {
        ...
        if (updated) {
          console.log(
            "[Admin] Bootstrap: first admin set from ADMIN_BOOTSTRAP_EMAIL",
          );
        }
      }
    }
