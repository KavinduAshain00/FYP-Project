...
function buildFormFromApi(full) {
  ...
  return {
    ...
    steps: Array.isArray(full.steps) ? full.steps.map((s) => normalizeStepForForm(s)) : [],
  };
}
...
export default function AdminModuleEditor() {
  ...
  const missingEditId =
    !isNew && (!moduleId || moduleId === 'undefined');
  ...
  const curriculumPartsKey = useCallback((parts) => [...parts].sort().join(','), []);
    ...
    try {
      ...
      if (isNew) {
        ...
        const data = {
          ...
          content: '',
          moduleType: 'vanilla',
          starterCode: { html: '', css: '', javascript: '', jsx: '', serverJs: '' },
          objectives: [],
          hints: [],
          steps: [],
        };
        setForm(data);
        initialFormRef.current = JSON.parse(JSON.stringify(moduleFormComparable(data)));
        return;
      }
  ...
  const handleSave = async () => {
    ...
    if (order !== wantedOrder) {
      toast.info(`Order ${wantedOrder} was already used; saved as ${order} (unique).`);
    }
  ...
  const handleGenerateCurriculumParts = async (parts) => {
    ...
    try {
      ...
      if (starterCode && typeof starterCode === 'object') done.push('starter code');
      toast.success(done.length ? `Generated ${done.join(', ')}` : 'Nothing new was applied');
    } catch (err) {
